...
<div className='grid grid-flow-col gap-1' style={{gridTemplateRows: 'repeat(7, minmax(0, 1fr)'}}>
    ...
    calenderGrid.map((day, index)=>{
        ...
        return <Link href={`/posts?date=${day}`} className='w-4 h-4 rounded cursor-pointer bg-gray-400' title={`${activityCount} Posts on ${day}`} style={{backgroundColor: `${activityCount == 0 ? "#ffffff10" : String(color)}`}}></Link>
    })
    }
</div>
)
